...
func (user *User) CreateThread(topic string) (conv Thread, err error) {
	...
	stmt, err := Db.Prepare("select id, uuid, topic, user_id, created_at from threads where uuid = ?")
	if err != nil {
		return
	}
	defer stmt.Close()
	...
	// use QueryRow to return a row and scan the returned id into the Session struct
	err = stmt.QueryRow(uuid).Scan(&conv.Id, &conv.Uuid, &conv.Topic, &conv.UserId, &conv.CreatedAt)
	return
}
